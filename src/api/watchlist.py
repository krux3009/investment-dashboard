"""Watchlist domain logic — code resolution + per-code marks.
...
Lives in the domain layer so both the HTTP route (`routes/watchlist.py`)
and the SSE broadcaster (`realtime.py`) can share it without importing
route modules.
"""
...
# Market suffix → native currency. Seeds dividends.fetch_one for codes
...
def _fetch_user_watchlist() -> list[str] | None:
    ...
    try:
        ...
        ret, data = anomalies.quote_ctx().get_user_security(group)
...
def market_ticker(code: str) -> tuple[str, str]:
    if "." in code:
...
def get_mark(code: str) -> WatchlistMark:
    """Per-code earnings + ex-div marks (yfinance-backed, 24h-cached)."""
    today = date.today()
    market, ticker = market_ticker(code)
    currency = _MARKET_CURRENCY.get(market, "USD")

    # Earnings — cached 24h via the earnings KV table.
    earnings_date: str | None = None
    earnings_days: int | None = None
    try:
        ep = earnings.fetch_next(code)
        if ep:
    ...
    try:
        dp = dividends.fetch_one(code, currency, ticker)
