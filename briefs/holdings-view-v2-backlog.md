...
4. **Phase 4 — watchlist view** ✅ shipped (this commit). `views/watchlist.py` renders a compact table beneath holdings with: ticker · last · 30d change · sparkline. Default tickers `US.NVDA, US.TSLA, HK.00700`; override via `MOOMOO_WATCHLIST` env. Reuses the prices cache from chunk 1 and the sparkline visual from chunk 2 (slight duplication of the `_sparkline` helper in watchlist.py vs holdings.py — extract to `views/charts.py` if a third caller appears). No own poll: rendered alongside holdings on the existing 30s callback so watchlist refreshes naturally without an extra dcc.Interval.

   **Update post-ship — pull from moomoo "All" group.** Hardcoded list was a placeholder. `_fetch_user_watchlist()` now calls `OpenQuoteContext.get_user_security('All')` and uses the codes verbatim — 23 tickers on this machine (PLTR/ANET/VRT/NBIS/MU/INTC + GOOG/GOOGL/NVDA/AMD/ELV/SNDK/STX/CIFR/HK.02513/...). Resolution order: `MOOMOO_WATCHLIST` env (explicit list) > moomoo group fetch (cached per session) > hardcoded `_DEFAULT_WATCHLIST`. Switch group via `MOOMOO_WATCHLIST_GROUP` (default "All") — useful once the user starts curating "Favorites" or a CUSTOM-typed group in moomoo's app. No de-dup against held positions: surprising filtering would worsen UX more than minor visual repetition does.

   **Update post-ship — watchlist rows are interactive.** Each watchlist row is now a clickable button (mirrors the holdings row pattern). Click expands a drill-in inline with: a watchlist-appropriate field summary line (`Last $X · 90d ↑ Y% · 90d hi $Z · 90d lo $Z`), a 90-day price chart (reuses `_drillin_chart` from holdings.py), and any TECHNICAL / Capital flow anomaly prose for that ticker. New `dcc.Store("watchlist-expanded")` tracks expansion state; new `_toggle_watchlist_row` callback in views/watchlist.py mirrors the holdings toggle. The render callback's Input list grew by one to fire re-renders when the watchlist expansion changes. The two states (holdings-expanded vs watchlist-expanded) stay decoupled so collapsing one doesn't affect the other.
